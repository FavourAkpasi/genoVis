...
const ChartFrame = ({ children }: { children: React.ReactNode }) => (
  <Card>
    <CardHeader>
      ...
      <p className="text-xs text-muted-foreground">Click a bar to filter the whole view by that lineage.</p>
    </CardHeader>
          ...
          return (
            <li key={lineage}>
              <button
                ...
                className={cn(
                  'group grid w-full grid-cols-[5.5rem_1fr_auto] items-center gap-3 rounded-md px-1 py-1 text-left text-xs hover:bg-muted/50',
                  active && 'bg-muted'
                )}
              >
